...
function createLoaderObject(loader) {
  // 创建一个包含normal/pitch/raw/data等属性
  var obj = {
    path: null,
    query: null,
    fragment: null,
...
function iteratePitchingLoaders(options, loaderContext, callback) {
  ...
  if (loaderContext.loaderIndex >= loaderContext.loaders.length)
    // 在该函数内部,会调用iterateNormalLoaders
    return processResource(options, loaderContext, callback);
  // 首次执行,loaderIndex为0
  var currentLoaderObject = loaderContext.loaders[loaderContext.loaderIndex];

  // 递归思想: 当前loader对象若已经执行了此对象的pitch方法,则执行下一个loader对象的pitch方法
  if (currentLoaderObject.pitchExecuted) {
    loaderContext.loaderIndex++;
    return iteratePitchingLoaders(options, loaderContext, callback);
  }

  // 当所有的pitchloader执行完成后,开始加载loader模块, loadLoader中的handleResult会返回callback参数对应的处理函数
  loadLoader(currentLoaderObject, function (err) {
    if (err) {
      loaderContext.cacheable(false);
      return callback(err);
    }
    // 获取当前loader对象的pitch函数
    var fn = currentLoaderObject.pitch;
    // 标记当前loader对象以及被iteratePitchingLoaders处理了
...
exports.runLoaders = function runLoaders(options, callback) {
  ...
  // 调用createLoaderObject循环创建loader对象
